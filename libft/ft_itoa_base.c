...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/12 10:53:24 by bcastelo          #+#    #+#             */
/*   Updated: 2022/11/12 10:53:27 by bcastelo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_itoa_base_r(unsigned int n, char *number,
		char *base, unsigned int size)
{
	if (n >= size)
		ft_itoa_base_r(n / size, number, base, size);
...
char	*ft_itoa_base(int n, char *base)
{
	...
	if (base_size < 2)
		return (NULL);
	number = ft_calloc(33, sizeof(char));
	if (!number)
		return (NULL);
	if (n < 0)
	{
		ft_putdigit(number, '-');
		ft_itoa_base_r(-n, number, base, base_size);
	}
	else
		ft_itoa_base_r(n, number, base, base_size);
